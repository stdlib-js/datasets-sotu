	...
	if ( opts.name ) {
		for ( i = 0; i < opts.name.length; i++ ) {
			if ( !hasOwnProp( INDEX_NAME, opts.name[ i ] ) ) {
				throw new RangeError( format( '0NH3i,N6', 'name', opts.name.join(',') ) );
			}
	...
	if ( opts.year ) {
		for ( i = 0; i < opts.year.length; i++ ) {
			if ( !hasOwnProp( INDEX_YEAR, opts.year[ i ] ) ) {
				throw new RangeError( format( '0NH3i,N6', 'year', opts.year.join(',') ) );
			}
	...
	if ( opts.party ) {
		for ( i = 0; i < opts.party.length; i++ ) {
			if ( !hasOwnProp( INDEX_PARTY, opts.party[ i ] ) ) {
				throw new RangeError( format( '0NH3i,N6', 'party', opts.party.join(',') ) );
			}
