...
function validate( opts, options ) {
	if ( !isObject( options ) ) {
		return new TypeError( format( '0Bc2h', options ) );
	}
	if ( hasOwnProp( options, 'name' ) ) {
		...
		if (
			...
			return new TypeError( format( '0Bc3w', 'name', opts.name ) );
		}
	}
	if ( hasOwnProp( options, 'year' ) ) {
		...
		if (
			...
			return new TypeError( format( '0Bc3x', 'year', opts.year ) );
		}
	}
	if ( hasOwnProp( options, 'range' ) ) {
		...
		if (
			...
			return new TypeError( format( '0Bc3y', 'range', opts.range ) );
		}
	...
	if ( hasOwnProp( options, 'party' ) ) {
		...
		if (
			...
			return new TypeError( format( '0Bc3w', 'party', opts.party ) );
		}
	}
	return null;
}
