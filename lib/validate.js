...
function validate( opts, options ) {
	if ( !isObject( options ) ) {
		return new TypeError( format( '0NH2V', options ) );
	}
	if ( hasOwnProp( options, 'name' ) ) {
		...
		if (
			...
			return new TypeError( format( '0NH3j', 'name', opts.name ) );
		}
	}
	if ( hasOwnProp( options, 'year' ) ) {
		...
		if (
			...
			return new TypeError( format( '0NH3k', 'year', opts.year ) );
		}
	}
	if ( hasOwnProp( options, 'range' ) ) {
		...
		if (
			...
			return new TypeError( format( '0NH3l', 'range', opts.range ) );
		}
		if ( opts.range[ 0 ] < 1790 ) {
			return new RangeError( format( '0NH3m', 'range[0]', opts.range ) );
		}
		if ( opts.range[ 1 ] > 5000 ) {
			return new RangeError( format( '0NH3n', 'range[1]', opts.range ) );
		}
	}
	if ( hasOwnProp( options, 'party' ) ) {
		...
		if (
			...
			return new TypeError( format( '0NH3j', 'party', opts.party ) );
		}
	}
	return null;
}
