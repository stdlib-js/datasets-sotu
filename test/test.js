...
tape( 'the function returns an object array', function test( t ) {
	var out = sotu();
	t.strictEqual( isObjectArray( out ), true, 'returns expected value' );
	t.end();
});
...
tape( 'the `name` option can be used to retrieve SOTU addresses by specific Presidents', function test( t ) {
	...
	t.strictEqual( isObjectArray( out ), true, 'returns expected value' );
	t.strictEqual( out.length, 8, 'returns selected speeches' );

	for ( i = 0; i < out.length; i++ ) {
		t.strictEqual( out[ i ].name, 'Barack Obama', 'speech is from expected President' );
	}
	...
	t.strictEqual( isObjectArray( out ), true, 'returns expected value' );
	t.strictEqual( out.length, 16, 'returns selected speeches' );

	for ( i = 0; i < out.length; i++ ) {
		bool = ( out[ i ].name === 'William J Clinton' || out[ i ].name === 'Barack Obama' );
		t.strictEqual( bool, true, 'returns a speech by a specified President' );
	}
	t.end();
});
...
tape( 'the `party` option can be used to retrieve SOTU addresses from Presidents belonging to particular political parties', function test( t ) {
	...
	for ( i = 0; i < out.length; i++ ) {
		t.strictEqual( out[ i ].party, 'Democratic', 'speech is from a Democratic President' );
	}
	...
	for ( i = 0; i < out.length; i++ ) {
		...
		t.strictEqual( bool, true, 'speech is from a Democratic or Republican President' );
	}
	t.end();
});
...
tape( 'the `year` option can be used to retrieve SOTU addresses from particular years', function test( t ) {
	...
	t.strictEqual( isObjectArray( out ), true, 'returns expected value' );
	t.strictEqual( out[ 0 ].name, 'Barack Obama', 'speech is from expected President' );

	out = sotu({
		'year': [ 1998, 2009 ]
	});
	t.strictEqual( isObjectArray( out ), true, 'returns expected value' );
	t.strictEqual( out[ 0 ].name, 'William J Clinton', 'speech is from expected President' );
	t.strictEqual( out[ 1 ].name, 'Barack Obama', 'speech is from expected President' );
...
tape( 'the `range` option can be used to retrieve SOTU addresses of a given time period', function test( t ) {
	...
	t.strictEqual( isObjectArray( out ), true, 'returns expected value' );
	t.strictEqual( out.length, 16, 'returns speeches from 1980 to 1995' );

	out = sotu({
		'range': [ 1880, 1920 ]
	});
	t.strictEqual( isObjectArray( out ), true, 'returns expected value' );
	t.strictEqual( out.length, 41, 'returns speeches from 1880 to 1920' );

	out = sotu({
		'range': [ 1920, 1970 ]
	});
	t.strictEqual( isObjectArray( out ), true, 'returns expected value' );
	t.strictEqual( out.length, 52, 'returns speeches from 1920 to 1970' );

	out = sotu({
		'range': [ 1935, 1960 ]
	});
	t.strictEqual( isObjectArray( out ), true, 'returns expected value' );
	t.strictEqual( out.length, 27, 'returns speeches from 1935 to 1960' );
...
tape( 'if provided multiple options, the function returns the union', function test( t ) {
	...
	t.strictEqual( isObjectArray( out ), true, 'returns expected value' );
	t.strictEqual( out.length, 24, 'returned array has expected length' );
	...
	t.strictEqual( counters.name, 8, 'returns speeches by name' );
	t.strictEqual( counters.year, 2, 'returns speeches by year' );
	t.strictEqual( counters.range, 10, 'returns speeches by range' );
	t.strictEqual( counters.party, 4, 'returns speeches by party' );
